...
public class ListingService {
    ...
    public PagedResponse<ListingDto> getPublicListings(int page, int size, String sort,
        ...
        List<Listing> filtered = result.getContent().stream()
                .filter(l -> category == null || category.equalsIgnoreCase(l.getCategory()))
                .toList();

        Page<Listing> pageResult = new PageImpl<>(filtered, pageable, filtered.size());

        return PagedResponse.of(pageResult.map(listingMapper::toDto));
    }
